...
If you want just extend a little original template then you may use `Choices.Templates` to get access to
...
const example = new Choices(element, {
  callbackOnCreateTemplates: () => ({
    input: (...args) =>
      Object.assign(Choices.Templates.input(...args), { type: 'email' }),
  }),
});
```
